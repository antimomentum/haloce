...
create_default_firewall()
{
    ...
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "VALUE" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "SNMP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "NTP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "Source Engine Query" --algo kmp --to 65535 -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "getstatus" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "QUIC" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "SNMP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "MDNS" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "SRVLOC" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "ISAKMP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "NBDS" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "XDCMP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "SSDP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "RTCP" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "H.248" --algo kmp -j DROP)" | bash
    wait
    echo "$(iptables -t mangle -A PREROUTING -i eth0 -m string --string "H.323" --algo kmp -j DROP)" | bash
    wait

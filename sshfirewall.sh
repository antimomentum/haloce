...
create_default_firewall()
{
    ...
    wait
    echo "ipset create DDOS hash:ip" | bash
    wait
    echo "ipset create TEST2 hash:ip" | bash
    wait
    echo "ipset create TEST1 hash:ip" | bash
    wait
    echo "ipset create SSH hash:ip" | bash
    wait
    ...
    wait
    echo "iptables -t mangle -A PREROUTING -m set --match-set SSH src -j ACCEPT" | bash
    wait
    ...
    wait
    echo "iptables -A INPUT -m set --match-set SSH src -j ACCEPT" | bash
    wait
    echo "iptables -A INPUT -j DROP" | bash
    wait
    echo "iptables -A FORWARD -j DROP" | bash
    wait
